    ...
    <div class="w-[360px] px-4">
        ...
        <div class="flex flex-col items-center">
            ...
            <template v-if="sanCodeStatus === 0">
                <div class="mt-2">使用手机微信扫码绑定</div>
                <div class="my-4 text-base text-tx-placeholder">
                    绑定后可使用微信扫码登录
                </div>
            </template>
            <template v-if="sanCodeStatus === 1">
                <div class="mt-2 text-error">二维码失效</div>
                <div class="my-4 text-base">请在点击二维码刷新</div>
            </template>
            <template v-if="sanCodeStatus === 2">
                <div class="mt-2 text-error">扫码成功</div>
                <div class="my-4 mb-3 text-base">请在微信公众号中确认绑定</div>
            </template>
            <template v-if="sanCodeStatus === 3">
                <div class="mt-2 text-error">绑定成功</div>
                <div class="my-4 text-base">恭喜您绑定微信成功</div>
            </template>
        </div>
    </div>
</template>
