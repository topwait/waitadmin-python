...
class PostService:
    ...
    async def whole(cls) -> List[schema.AuthPostWholeVo]:
        ...
        """
        fields = ["id", "code", "name"]
        lists = await AuthPostModel.filter(is_delete=False).order_by("-sort", "-id").all().values(*fields)
    ...
    async def lists(cls, params: schema.AuthPostSearchIn) -> PagingResult[schema.AuthPostListVo]:
        ...
        _model = AuthPostModel.filter(is_delete=False).filter(*where).order_by("-sort", "-id")
    ...
    async def add(cls, post: schema.AuthPostAddIn):
        ...
        Author:
            zero
        """
        pc = await AuthPostModel.filter(code=post.code, is_delete=False)
        if pc:
            raise AppException("岗位编号已被占用")

        pn = await AuthPostModel.filter(name=post.name, is_delete=False)
        if pn:
    ...
    async def edit(cls, post: schema.AuthPostEditIn):
        ...
        Author:
            zero
        """
        _post = await AuthPostModel.filter(id=post.id, is_delete=False).first().values("id")
        if not _post:
            raise AppException("岗位不存在")

        _post2 = await AuthPostModel.filter(code=post.code, id__not=post.id, is_delete=False).values("id")
        if _post2:
            raise AppException("岗位编号已被占用")

        _post3 = await AuthPostModel.filter(name=post.name, id__not=post.id, is_delete=False).values("id")
        if _post3:
            raise AppException("岗位名称已被占用")
    ...
    async def delete(cls, id_: int):
        ...
        Author:
            zero
        """
        p = await AuthPostModel.filter(id=id_, is_delete=False).first().values("id")
        if not p:
            raise AppException("岗位不存在")

        admin = await AuthAdminModel.filter(post_id=id_, is_delete=False).first().values("id")
        if admin:
            raise AppException("岗位已被使用不能删除")

        await AuthPostModel.filter(id=id_).update(
            is_delete=True,
